...
#Imprting requirements
import tkinter
from tkinter import IntVar, END, DISABLED, NORMAL,Menu
...
root.iconbitmap('resources/icons/morse.ico')
root.resizable(0,0)
...
def play():
    ...
    for value in text:
        if value == ".":
            playsound('resources/voice_clips/dot.mp3')
            root.after(100)
        elif value == "-":
            playsound('resources/voice_clips/dash.mp3')
            root.after(200)
        elif value == " ":
            root.after(300)
        elif value == "|":
...
def show_guide():    
    ...
    guide.iconbitmap('resources/icons/morse.ico')
    guide.geometry('350x360+'+ str(root.winfo_x()+500) + "+" + str(root.winfo_y()))
    guide.config(bg=root_color)

    #Display morse chart
    morse = ImageTk.PhotoImage(Image.open('resources/images/morse_chart.jpg'))
...
my_menu.add_command(label="Guide",command=show_guide)                               

#guide_menu=Menu(my_menu,tearoff=0)
#my_menu.add_cascade(label="Guide",menu=guide_menu)
#guide_menu.add_command(label="Open Guide",command=show_guide)

#about_menu=Menu(my_menu,tearoff=0)
#my_menu.add_cascade(label="About",menu=about_menu)
#about_menu.add_command(label="Aboutsub",command=show_guide)
